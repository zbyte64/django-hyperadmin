...
class StoragePaginator(object):
    ...
    def get_links_and_instances(self):
        ...
        if self.path:
            ...
            link = Link(url=url, endpoint=self.endpoint, prompt=u"/", classes=['filter', 'directory'], rel="filter", group="directory")
            links.append(link)
        for directory in self.dirs:
            url = './%s' % self.state.get_query_string({'path':directory})
            link = Link(url=url, endpoint=self.endpoint, prompt=directory, classes=['filter', 'directory'], rel="filter", group="directory")
            links.append(link)
        if '/' in self.path:
